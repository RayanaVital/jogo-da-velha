...
export class JogoDaVelhaService {
  ...
  private readonly TAM_TAB: number = 3; //tamanho do tabuleiro, 3 linhas por 3 colunas
  private readonly X: number = 1; //peça numero 1
  private readonly O: number = 2;
  private readonly VAZIO: number = 0; //o tabuleiro trabalhara com 3 estados ou estara vazio ou com o 1 ou com 2


  private tabuleiro: any; //tabuleiro é um array multidimensional com 3 linhas por 3 colunas
  private numMovimentos: number; //delimita o numero maximo de jogadas permitidas
  private vitoria: any; //armazena a posições das vitorias
  ...
  get showInicio(): boolean { // criada pois _showInicio é uma propiedade privada mas precisara ser acessado pela view
    return this._showInicio; //padrão get do typescript cria um acessor pra ela encapsulando
  }
  ...
  jogar(posX:number, posY: number): void {
    //jogada invalida
    //verifica se o campo é diferente de vazio, se for o campo ja esta preenchido e a jogada é invalida
    //ou se alguem venceu e ficar apertando nos campos essa jogada sera considerada invalida
    if(this.tabuleiro[posX][posY] !== this.VAZIO || this.vitoria){
      return;
    }

    this.tabuleiro[posX][posY] = this._jogador; // o primeiro jogador é o x
    this.numMovimentos++;
    this.vitoria = this.fimJogo(posX, posY,
      this.tabuleiro, this._jogador);
      this._jogador = (this._jogador === this.X) ? this.O : this.X;
      //se o jogador que jogou agora for o x então sera o 0, caso contrario muda pro x
  ...
  fimJogo(linha: number, coluna: number, tabuleiro: any, jogador: number){
    ...
    //valida as diagonais
    if(tabuleiro [0][0] === jogador && tabuleiro [1][1] === jogador && [2][2] === jogador){
      fim = [[0,0],[1,1],[2,2]];
    }
     if(tabuleiro [0][2] === jogador && tabuleiro [1][1] === jogador && [2][0] === jogador){
      fim = [[0,2],[1,1],[2,0]];
    }
  ...
  cpuJogar(): void{
    ...
    this.tabuleiro[jogada[0]][jogada[1]] = this._jogador; //joga
    this.numMovimentos++; //incrementa o numero de movimentos
    this.vitoria = this.fimJogo(jogada[0], jogada[1], //verifica se a jogada é o suficiente para ganhar o jogo
      this.tabuleiro, this._jogador);
    this._jogador = (this._jogador === this.X) ? this.O : this.X;// inverte o jogador para passar a vez pro jogador x
  }
  ...
  obterJogada(jogador: number): number[] { //itera sobre o tabuleiro para saber se o jogo pode terminar ou não
